...
basic.forever(function () {
    ...
    if (game.score() == 10) {
        bonus = game.createSprite(random, -1)
    }
...
difficult += 1
    while (count < 5 && ++count) {
        alive = !(enemy.isDeleted())
        destroyed = enemy.isDeleted()
...
if (difficult <= 10) {
    ...
    function tir() {
        ...
        while (count2 < 4 && ++count2) {
            ...
            let enemyY2 = enemy.get(LedSpriteProperty.Y);
            let enemyX = enemy.get(LedSpriteProperty.X);

            if ((shootY == enemyY2) && (shootX == enemyX)) {
